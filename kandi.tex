...
\abstract
{Ohjelmistoihin joudutaan virhetilanteiden ja ominaisuuksien lisäysten takia tekemään muutoksia. Usein muutoksen tai korjauksen tekee ohjelmoija, joka ei alunperin ole ollut tuottamassa kyseistä ohjelmakoodia. Ohjelmoija pyrkii selvittämään järjestelmän toimintaa, jotta hän pääsisi nopeasti tekemään halutun muutoksen. Usein ohjelmoija esittää itselleen kysymyksiä selvittääkseen ohjelmiston logiikkaa ja toimintaa.

Ohjelmoijan tueksi on kehitetty useita erilaisia työkaluja ja ohjelmistoja. Moni ei kuitenkaan osaa käyttää näitä työkaluja täysimääräisesti ja tehokkaasti osana omaa ohjelmointiprosessiaan.
...
StackOverflown toiminta perustuu käyttäjien aktiivisuuteen ja tutkimuksissa on havaittu, että aktiiviset StackOverflowssa kysymyksiin vastaavat käyttäjät ovat myös aktiivisia GitHubin käyttäjiä. Kun taak GitHubissa passiivisemmat ovat aktiivisempia kysymään kysymyksiä StackOverflowssa~\cite{stackoverflow-and-github}.

StackOverflowssa useiden kysymysten ohessa on toimimaton koodiesimerkki, johon kysymys liittyy. Ja vastaavasti monissa vastauksissa on tarjolla toimiva koodiesimerkki. Esimerkit ovat kuitenkin vaihtelevasti toimivia, ja satunnaisesti esimerkeistä löytyy virheitä. Tämän vuoksi on tärkeää, että yhteisö StackOverflowssa arvioi toistensa vastauksia. Näin hyvät vastaukset nousevat heikompien ylle ja käyttäjien on helppo tunnistaa hyvät vastaukset ja niissä olevat toimivat koodiesimerkit.
...
Wikipohjainen dokumentaatio on useasti helpommin myös ohjelmiston käyttäjien muokattavissa, sillä muutokset tavanomaisesti tehdään suoraan internet-selaimeslla eikä ohjelmiston lähdekoodia tarvitse erikseen ladata muokatakseen dokumentaatiota. Näin wikit sallivat ohjelmiston käyttäjien helpommin muokata ja kehittää ohjelmiston dokumentaatiota.
