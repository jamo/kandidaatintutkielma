...
Projektien koon kasvaessa ja yhä kehittyneempien koodin hakumahdollisuuksien myötä koodin etsimisestä on muodostunut yhä oleellisempi vaihe sekä ohjelmatuotannossa että ylläpitovaiheessa. Sadowski, Stolee ja Elbaum esittelevä artikkelissaan ''How Developers Search for Code: A Case Study'' \cite{g_search_code}, kuinka ohjelmoijat etsivät koodia Googlella.
...
Kirjoittajat kertovat artikkelissaan, että Googlella suurin osa ohjelmakoodista on yhdessä isossa repositoriossa, josta kaikki pääsevät katsomaan ja halutessaan hyödyntämään toistensa koodia. Googlella on myös käytössä sisäinen koodin hakutyökalu (code search tool), jossa hakua voi rajata mm. kielen ja sijainnin perusteella. Hakutulosten näyttämisen lisäksi koodihaku mahdollistaa koodin läpi navigoinnin kansiorakenteen sekä koodiviittausten perusteella. %viittaus siis kuten ctrl+click NBssä keywordin kohdalla.
...
Tapaustutkimuksesta saatu data erikseen ja niiden yhdiste mahdollistivat tarkemman analyysin siihen, miten koodia etsitään. He myös havaitsivat, että koodia etsitään aktiiviesti, koodiahakua käytetään paljon esimerkkien sekä etsivät pääosin jo itselleen tutusta koodista. He myös havaitsivat, että ohjelmoija useasti tarkentaa hakutermejään iteratiivisesti.
\begin{enumerate}
...
\begin{itemize}
    ...
    \item Tarjoa tulokset huomioiden monipuolisempi konteksti, mahdollisesti jopa seuraamalla käyttäjän toimia muualla, mm. käydyt keskusteluy.
    \item Harkitse työkalun integraatiota kehytysympäristöön.
\end{itemize}

Sadowski, Stolee ja Elbaum muistuttavat vielä lopuksi, että Googlen uniikki toimintatapa saattaa vaikuttaa tuloste soveltuvuuteen ja toistettavuuteen.
